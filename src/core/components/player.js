        ...
        if(key != CS1.socket.id){
          if(CS1.otherPlayers[key]){
            ...
            if(    op.avatarType===2 &&   op.HEAD  &&  (op.device === 'Oculus Quest')   ){
              ...
              op.HAND_L.object3D.position.set(-1.5*o[key].lhp.x , 1.5*o[key].lhp.y+0.7, -3*o[key].lhp.z);
              ...
              op.HAND_R.object3D.position.set(-1.5*o[key].rhp.x  , 1.5*o[key].rhp.y+0.7 , -3*o[key].rhp.z); 
...
//         }
    ...
    if(++this.totalSteps%6 == 0) {
      ...
      if(   (CS1.device == 'Oculus Quest') &&  (CS1.myPlayer._avatarType === 2)  && this.lh.loaded ){
