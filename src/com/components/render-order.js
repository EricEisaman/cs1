...
AFRAME.registerComponent('render-order', {  
    ...
    init: function () {    
        ...
        this.el.addEventListener('object3dset', evt => {      if (this.id !== 'nonrecursive') {        evt.detail.object.traverse(this.set);      }    });  },  
    update: function () {    
            if (this.id === 'nonrecursive') {      
                this.set(this.el.object3D);    } 
                else {      this.el.object3D.traverse(this.set);    }  },  
    set: function (node) {    
        if (isNaN(this.data)) {      
...
AFRAME.registerComponent('render-order-recursive', {  
    init: function () {    
        this.el.addEventListener('child-attached', evt => {evt.detail.el.setAttribute('render-order', this.el.getAttribute('render-order'));    });  
    }
});
